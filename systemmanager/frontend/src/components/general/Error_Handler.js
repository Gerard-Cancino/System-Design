      ...
      res==undefined?(
        ...
        res.data!= undefined?(
          res.data.data!=undefined?(
            <div className="row alert alert-success alert-dismissible show mb-0" role="alert">
              {res.data.message}
            </div>
          ):(
            <div className="row alert alert-success alert-dismissible show mb-0" role="alert">
          ...
          400 <= parseInt(res.response.status) && parseInt(res.response.status) <= 452?(
            res.response.data.non_field_errors!=undefined && res.response.data.non_field_errors.length != 0?(
              <div className="row alert alert-danger alert-dismissible show mb-0" role="alert">
